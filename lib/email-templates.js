...
function linkForValue(value) {
  ...
  if (/^https?:\/\/[^\s]+$/i.test(raw)) {
    return { href: raw, label: 'Otvoriť odkaz ↗', kind: 'url' };
  }

  if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(raw)) {
    return { href: `mailto:${raw}`, label: raw, kind: 'email' };
  }

  if (/^\+?[\d\s()./-]{7,}$/.test(raw)) {
    const phone = raw.replace(/(?!^)\+/g, '').replace(/[^\d+]/g, '');
    if (phone.replace(/\D/g, '').length >= 7) {
      return { href: `tel:${phone}`, label: raw, kind: 'phone' };
    }
  }
...
function renderValue(value, theme = 'admin') {
  const link = linkForValue(value);
  if (!link) return escapeMultiline(value);

  const color = theme === 'admin' ? '#f4d486' : '#7a5816';
  const decoration = link.kind === 'url' ? 'none' : 'underline';
  return `<a href="${escapeText(link.href)}" style="color:${color};font-weight:700;text-decoration:${decoration};text-underline-offset:2px;">${escapeText(link.label)}</a>`;
}

function renderAdminRows(items) {
  return normalizeItems(items).map((item, index) => `
    <tr>
      <td width="34%" valign="top" style="width:34%;padding:${index ? '14px' : '0'} 14px 14px 0;${index ? `border-top:1px solid ${COLORS.adminBorder};` : ''}font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.45;font-weight:700;letter-spacing:.7px;text-transform:uppercase;color:${COLORS.adminMuted};">
        ${escapeText(item.label)}
      </td>
      <td valign="top" style="padding:${index ? '14px' : '0'} 0 14px 10px;${index ? `border-top:1px solid ${COLORS.adminBorder};` : ''}font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.55;color:${COLORS.adminText};word-break:break-word;overflow-wrap:anywhere;">
        ${renderValue(item.value, 'admin')}
      </td>
    </tr>`).join('');
}
...
function renderCustomerRows(items) {
  return normalizeItems(items).map((item, index) => `
    <tr>
      <td width="34%" valign="top" style="width:34%;padding:${index ? '13px' : '0'} 14px 13px 0;${index ? `border-top:1px solid ${COLORS.customerBorder};` : ''}font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.45;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:${COLORS.customerMuted};">
        ${escapeText(item.label)}
      </td>
      <td valign="top" style="padding:${index ? '13px' : '0'} 0 13px 10px;${index ? `border-top:1px solid ${COLORS.customerBorder};` : ''}font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.55;color:${COLORS.customerText};word-break:break-word;overflow-wrap:anywhere;">
        ${renderValue(item.value, 'customer')}
      </td>
    </tr>`).join('');
}
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${COLORS.adminSection}" style="width:100%;border-collapse:separate;background-color:${COLORS.adminSection};border:1px solid ${COLORS.adminBorder};border-left:4px solid ${COLORS.accent};border-radius:5px;mso-table-lspace:0pt;mso-table-rspace:0pt;">
          <tr>
            <td style="padding:15px 17px;">
              ...
              <div style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:18px;line-height:1.35;font-weight:700;color:${COLORS.white};word-break:break-word;overflow-wrap:anywhere;">${renderValue(item.value, 'admin')}</div>
            </td>
          </tr>
        </table>
      </td>
    ...
    <tr>
      <td style="padding:0 28px 18px 28px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#f7f8fa" style="width:100%;border-collapse:separate;background-color:#f7f8fa;border:1px solid ${COLORS.customerBorder};border-radius:6px;mso-table-lspace:0pt;mso-table-rspace:0pt;">
          <tr>
            <td style="padding:18px 20px 4px 20px;">
              <div style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.4;font-weight:700;letter-spacing:.9px;text-transform:uppercase;color:#9a7120;">${escapeText(section.title)}</div>
            </td>
          </tr>
          <tr>
            <td style="padding:12px 20px 5px 20px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${COLORS.adminPanel}" style="width:100%;max-width:640px;border-collapse:separate;background-color:${COLORS.adminPanel};border:1px solid #242e39;border-radius:8px;overflow:hidden;mso-table-lspace:0pt;mso-table-rspace:0pt;">
              ...
              <td bgcolor="#0d131a" style="padding:28px;border-bottom:3px solid ${COLORS.accent};background-color:#0d131a;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;">
                  <tr>
                    ...
                    <td align="right" valign="middle" style="font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.3;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:${COLORS.accent};">${safeType}</td>
                  </tr>
                </table>
              </td>
            </tr>
            <tr>
              <td style="padding:28px 28px 20px 28px;">
                ${safeReference ? `<div style="display:inline-block;margin:0 0 12px 0;padding:6px 10px;border:1px solid #735f2f;border-radius:3px;background-color:#211d14;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1;font-weight:700;letter-spacing:.8px;color:${COLORS.accent};">${safeReference}</div>` : ''}
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${COLORS.white}" style="width:100%;max-width:640px;border-collapse:separate;background-color:${COLORS.white};border:1px solid #d9dee4;border-radius:8px;overflow:hidden;mso-table-lspace:0pt;mso-table-rspace:0pt;">
            <tr>
              <td bgcolor="#0d131a" style="padding:28px;background-color:#0d131a;border-bottom:3px solid ${COLORS.accent};">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;">
                  <tr>
                    <td valign="middle" style="font-family:Arial,Helvetica,sans-serif;font-size:24px;line-height:1;font-weight:800;letter-spacing:2px;color:${COLORS.white};">PP <span style="color:${COLORS.accent};">AUTO</span></td>
                    <td align="right" valign="middle" style="font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.3;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#d9e0e7;">${escapeText(typeLabel, 180)}</td>
                  </tr>
                </table>
              </td>
            </tr>
            <tr>
              <td style="padding:30px 28px 22px 28px;">
                ${safeReference ? `<div style="display:inline-block;margin:0 0 13px 0;padding:6px 10px;border:1px solid #e1cc99;border-radius:3px;background-color:#fbf7ec;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1;font-weight:700;letter-spacing:.8px;color:#84621b;">${safeReference}</div>` : ''}
            ...
            <tr>
              <td align="center" style="padding:0 28px 30px 28px;">
                <a href="${escapeText(safePrimaryHref)}" target="_blank" rel="noopener" style="display:inline-block;padding:13px 22px;border-radius:4px;background-color:#111820;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1;font-weight:700;letter-spacing:.5px;text-decoration:none;color:${COLORS.white};">${escapeText(primaryLabel, 100)}</a>
              </td>
            </tr>
            <tr>
              <td bgcolor="#0d131a" style="padding:22px 28px;background-color:#0d131a;border-top:3px solid ${COLORS.accent};">
                ...
                <p style="margin:0 0 7px 0;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.65;color:#a5afb9;">Partizánska 5660/107 · 058 01 Poprad<br><a href="tel:+421903905280" style="color:#d7dde4;text-decoration:none;">+421 903 905 280</a> · <a href="mailto:predaj@ppauto.sk" style="color:#d7dde4;text-decoration:none;">predaj@ppauto.sk</a></p>
